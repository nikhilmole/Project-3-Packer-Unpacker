...
class MarvellousLogin extends Template implements ActionListener, Runnable 
{
    ...
    private String encrypt(String valueToEnc) throws Exception 
    {
        ...
        cipher.init(Cipher.ENCRYPT_MODE, skeySpec, new IvParameterSpec(INIT_VECTOR.getBytes()));
    ...
    private String decrypt(String encrypted) throws Exception 
    {
        SecretKeySpec skeySpec = new SecretKeySpec(SECRET_KEY.getBytes(), "AES");
        Cipher cipher = Cipher.getInstance("AES/CBC/PKCS5PADDING");
        cipher.init(Cipher.DECRYPT_MODE, skeySpec, new IvParameterSpec(INIT_VECTOR.getBytes()));
    ...
    public void actionPerformed(ActionEvent ae) 
    {
        ...
        if (ae.getSource() == SUBMIT) 
        {
            ...
            try 
            {
                String encryptedPassword = encrypt("MarvellousAdmin");
                String decryptedPassword = decrypt(encryptedPassword);

                if (value1.equals("MarvellousAdmin") && value2.equals(decryptedPassword)) 
                {
